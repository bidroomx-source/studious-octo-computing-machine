<header className="bg-[#1e3a5f] text-white py-6 sticky top-0 z-50 shadow">
  <div className="max-w-6xl mx-auto px-6 flex justify-between items-center">
    <h1 className="text-4xl font-black tracking-tight">BidRoom</h1>
    <nav className="flex gap-6 text-lg md:gap-8 items-center">
      <a href="/browse" className="hover:text-[#c9a227] transition hidden md:block">Browse Rooms</a>
      <button onClick={() => setShowSellModal(true)} className="hover:text-[#c9a227] transition font-medium">Sell</button>
      <a href="#" className="hover:text-[#c9a227] transition hidden md:block">How it Works</a>
    </nav>
  </div>
</header>